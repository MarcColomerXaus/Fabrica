...
// Funció que construeix una solució aleatòria tenint en compte les penalitzacions afegides i la probabilitat alfa.
// FUNCIONA BÉ
vector<int> construir_solucio_aleatoria()
{
    ...
    --no_usats[solucio[0]];
    // Agafem una alfa aleatòria entre 1 i dades.K:
    ...
    for (int i = 1; i < dades.C; ++i) {
        ...
        priority_queue<candidat, vector<candidat>, greater<candidat>> llista_candidats;
        for (int j = 0; j < dades.K; ++j) {
            if (no_usats[j] > 0) {
                int penalitzacio = calcular_penalitzacio_afegida(solucio, j);
                llista_candidats.push({penalitzacio, j});
            }
        }
        // Triem un nombre al atzar de la cua entre els alpha primers números:
        int classe_triada;
        if (llista_candidats.size() < alpha)
            classe_triada = rand() % llista_candidats.size();
        else
            classe_triada = rand() % alpha;

        for (int i = 0; i < classe_triada; ++i) {
            llista_candidats.pop();
        }
        solucio[i] = llista_candidats.top().second;
        --no_usats[solucio[i]];
    }
    return solucio;
}
